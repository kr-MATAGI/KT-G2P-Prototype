...
def load_dataset(mode):
    ...
    if mode == 'train':
        train_file = os.path.join(data_dir, 'merged2_train.xlsx')
        train_data = pd.read_excel(train_file)

        valid_file = os.path.join(data_dir, 'merged2_val.xlsx')
        ...
        test_file = os.path.join(data_dir, 'merged2_test.xlsx')
...
def make_iter(batch_size, mode, train_data=None, valid_data=None, test_data=None):
    ...
    """
    path = "model/transformers/"
    # path = "./"
    # load text and label field made by build_model/transformers/pickles.py
    file_kor = open(path+'pickles/kor.pickle', 'rb')
    # file_kor = open('model/transformers/pickles/kor.pickle', 'rb')
    kor = pickle.load(file_kor)

    file_eng = open(path+'pickles/eng.pickle', 'rb')
    # file_eng = open('model/transformers/pickles/eng.pickle', 'rb')
...
class Params:
    ...
    def load_vocab(self):
        ...
        path = "model/transformers/"
        # path = "./"
        pickle_kor = open(path+'pickles/kor.pickle', 'rb')
        kor = pickle.load(pickle_kor)

        pickle_eng = open(path+'pickles/eng.pickle', 'rb')
