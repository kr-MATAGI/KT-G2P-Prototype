...
class Trainer:
    def __init__(self, params, mode, train_iter=None, valid_iter=None, test_iter=None, patience=50):
        self.params = params
        ...
        #     optim.Adam(self.model.parameters(), betas=(0.9, 0.998), eps=1e-9, lr=0.0002),
        #     hidden_dim=params.hidden_dim,
        #     warm_steps=params.warm_steps
        # )


        self.optimizer = optim.Adam(self.model.parameters(), betas=(0.9, 0.998), lr=0.0002, weight_decay=1e-4)
        ...
        self.criterion = nn.CrossEntropyLoss(ignore_index=self.kor.vocab.stoi["<pad>"])
    ...
    def inference(self):
            ...
            for batch in self.test_iter:
                ...
                for i in range(target.shape[0]):
                    ...
                    if pred_translation == target_translation:
                        ...
                        # 틀린 번역을 incorrect_translations 리스트에 추가
                        incorrect_translations.append({
                            'Source': source_translation,
                            'Predicted': join_jamos(pred_translation),
                            'Target': join_jamos(target_translation)
                        })
                #
                #     print(
        ...
        # incorrect_translations 리스트를 DataFrame으로 변환
        incorrect_df = pd.DataFrame(incorrect_translations)

        # xlsx 파일로 저장
        incorrect_df.to_excel('incorrect_translations.xlsx', index=False)
